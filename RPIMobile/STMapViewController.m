...
- (void)loadView {
    self.navigationItem.title = @"Shuttles";
    _isWest = TRUE;
    _hasLocation = FALSE;
    
    //Added for the Union, the only shuttle stop that both routes cover
    _unionButton = [[UIBarButtonItem alloc] init];
    _unionButton = [[UIBarButtonItem alloc] initWithTitle:@"West" style:UIBarButtonItemStylePlain target:self action:@selector(unionButtonAction:)];
    ...
    //Added for iOS 7: Disables rotating so that buses are facing the proper direction
    [self.mapView setRotateEnabled:FALSE];
    [self.mapView setPitchEnabled:FALSE];
    
    self.view = self.mapView;
    
    //Start Timer for Update Title
...
#pragma mark - Shuttle Times Tracker
...
- (int)timeForStop:(NSString *)stopString {
    //Determine Day of the week
    NSDate *today = [NSDate date];
    NSDateFormatter *myFormatter = [[NSDateFormatter alloc] init];
    [myFormatter setDateFormat:@"c"]; // day number, like 7 for saturday
    NSString *dayOfWeek = [myFormatter stringFromDate:today];
    int numberOfWeek = [dayOfWeek intValue];
    
    NSString *fileName;
    //Saturday
    if (numberOfWeek == 7) {
        fileName = [@"Saturday " stringByAppendingString:stopString];
    }
    //Sunday
    else if (numberOfWeek == 1) {
        fileName = [@"Sunday " stringByAppendingString:stopString];
    }
    //Weekday
    else {
        fileName = [@"Weekday " stringByAppendingString:stopString];
    }
    ...
    NSError *error;
    NSArray *stopTimes = [[NSString stringWithContentsOfFile:filePath encoding:NSUTF8StringEncoding error:&error] componentsSeparatedByString:@"\n"];
    for (unsigned int i = 0; i < [stopTimes count]; i++) {
        NSArray *timeArray = [[stopTimes objectAtIndex:i] componentsSeparatedByString:@":"];
        if ([timeArray count] >= 2) {
            NSInteger tempHour = [[timeArray objectAtIndex:0] integerValue];
            NSInteger tempMinute = [[timeArray objectAtIndex:1] integerValue];
            if ((tempHour == hour && tempMinute > minute) || tempHour > hour) {
                if (tempHour == hour) {
                    ETA = tempMinute-minute;
                }
                else {
                    ETA = 60 - minute;
                    ETA = ETA + tempMinute;
...
- (void)unionButtonAction:(id)sender {
    if ([_unionButton.title isEqualToString:@"West"]) {
        [_unionButton setTitle:@"East"];
        _isWest = FALSE;
    }
    else if ([_unionButton.title isEqualToString:@"East"]) {
        [_unionButton setTitle:@"West"];
        _isWest = TRUE;
    }
    [self updateTitle];
}
...
- (void)updateTitle {
    //If We don't have the location yet, do nothing
    if (!_hasLocation) {
        return;
    }
    
    //Find Closest Stop
    CLLocationDistance closestDistance = 1000000000;
    STStop *closestStop;
    for (unsigned int i = 0; i < [_stops count]; i++) {
        STStop *stop = (STStop *)[_stops objectAtIndex:i];
        NSLog(@"Name: %@", stop.name);
        CLLocation *currentLoc = self.mapView.userLocation.location;
        CLLocation *stopLoc = [[CLLocation alloc] initWithLatitude:[stop.latitude floatValue] longitude:[stop.longitude floatValue]];
        CLLocationDistance distance = [stopLoc distanceFromLocation:currentLoc];
    ...
    NSString *name = closestStop.name;
    if ([closestStop.name isEqualToString:@"Student Union"]) {
        self.toolbarItems = @[_locationButton, _flexibleSpace, _unionButton];
        if (_isWest) {
            name = [name stringByAppendingString:@" West"];
        }
        else {
            name = [name stringByAppendingString:@" East"];
        }
    }
    else {
        self.toolbarItems = @[_locationButton];
    ...
    //Output the ETA on that stop
    UILabel *label = (UILabel *)self.navigationItem.titleView;
    int ETA = [self timeForStop:name];
    if (ETA == 0) {
        label.text = @"Shuttles Have Stopped Running";
    }
    else {
        label.text = [NSString stringWithFormat:@"%@: %i Minutes", name, ETA];
    }
    [label sizeToFit];
}
